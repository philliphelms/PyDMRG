...
from pydmrg.efficient.lib.linalg_helper import eig # Library containing davidson algorithm
...
for i in range(int(N)-1,0,-1):
    tmp_sum1 = einsum('cdf,eaf->acde',F[i+1],M[i])
    tmp_sum2 = einsum('ydbe,acde->abcy',W[i],tmp_sum1)
    F[i] = einsum('bxc,abcy->xya',np.conj(M[i]),tmp_sum2)
...
# Right Sweep ----------------------------
    ...
    for i in range(N-1):
        ...
        def opt_fun(x): # Function to be called by Davidson algorithm
            ...
            in_sum1 = einsum('ijk,lmk->ijlm',F[i+1],x_reshape)
            in_sum2 = einsum('njol,ijlm->noim',W[i],in_sum1)
            fin_sum = einsum('pnm,noim->opi',F[i],in_sum2)
        ...
        u,v = eig(opt_fun,init_guess,precond,nroots=1)
        # select eigenvalue
...
# Left Sweep -----------------------------
    ...
    for i in range(N-1,0,-1):
        ...
        def precond(dx,e,x0):
            return dx / (diag - e)
        init_guess = np.reshape(M[i],-1)
        u,v = eig(opt_fun,init_guess,precond,nroots=1)#min(target_state+1,n1*n2*n3-1))
