...
#print(np.dot(lwf_ed,lwf_ed))
#print(np.dot(rwf_ed,rwf_ed))
#print(np.dot(lwf_ed,rwf_ed))
...
for i in range(int(N)-1,0,-1):
    F[i] = np.einsum('bxc,ydbe,eaf,cdf->xya',np.conj(Ml[i]),W[i],M[i],F[i+1])
##############################################
...
# Right Sweep ----------------------------
    ...
    for i in range(N-1):
        ...
        # Update F
        F[i+1] = np.einsum('jlp,ijk,lmin,npq->kmq',F[i],np.conj(Ml[i]),W[i],M[i])
# Left Sweep -----------------------------
