...
Nx = int(argv[2])
p = 0.1 
mbd = np.array([int(argv[3])])
bc = str(argv[4]) # Periodic, Open, Closed
...
for sind,s in enumerate(sVec):
    if bc == 'periodic':
        ...
        mpo = return_mpo_asep2D((Nx,Ny), (0.5,  # jr
                                          ...
                                          periodicx=False)
    if sind == 0:
        E[sind,:],EE[sind,:],gap[sind,:] = run_dmrg(mpo,mbd=mbd,fname=fname,nStates=2,alg='arnoldi')
    else:
        E[sind,:],EE[sind,:],gap[sind,:] = run_dmrg(mpo,mbd=mbd,initGuess=fname,fname=fname,nStates=2,alg='arnoldi')
    # Save Results
