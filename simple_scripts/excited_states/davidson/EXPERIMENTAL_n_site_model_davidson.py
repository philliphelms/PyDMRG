...
# Right Sweep ----------------------------
    ...
    for i in range(N-1):
        ...
        def opt_fun(x): # Function to be called by Davidson algorithm
            ...
            in_sum1 = np.einsum('ijk,lmk->ijlm',F[0][i+1],x_reshape) # HOW DO YOU PICK WHICH F TO WORK WITH???
        ...
        for j in range(target_state+1):
            M[j][i] = np.reshape(v[sort_inds[min(j,len(u)-1)]],(n1,n2,n3))
            # Right Normalize
            M_reshape = np.reshape(M[j][i],(n1*n2,n3))
            (U,s,V) = np.linalg.svd(M_reshape,full_matrices=False)
            M[j][i] = np.reshape(U,(n1,n2,n3))
            M[j][i+1] = np.einsum('i,ij,kjl->kil',s,V,M[j][i+1])
            # Update F
            F[j][i+1] = np.einsum('jlp,ijk,lmin,npq->kmq',F[j][i],np.conj(M[j][i]),W[i],M[j][i])
# Left Sweep -----------------------------
