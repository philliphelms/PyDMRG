...
Mr_exct = [] # Ordering (sigma,a_0,a_1)
Ml_exct = []
...
Mr_exct.insert(0,np.reshape(psir,(2,1,min(2,maxBondDim))))
Ml_exct.insert(0,np.reshape(psil,(2,1,min(2,maxBondDim))))
##############################################

# Random Start of Mr & Ml ####################
Mr = [None]*len(Mr_exct)
Ml = [None]*len(Ml_exct)
for i in range(len(Mr)):
    (nx,ny,nz) = Mr_exct[i].shape
    Mr[i] = np.random.rand(nx,ny,nz)
    Ml[i] = np.random.rand(nx,ny,nz)
##############################################
...
for i in range(2**N):
    ...
    tmp_matl = np.array([[1]])
    for k in range(N):
        tmp_matr = np.einsum('ij,jk->ik',tmp_matr,Mr_exct[k][i_occ[k],:,:])
        tmp_matl = np.einsum('ij,jk->ik',tmp_matl,Ml_exct[k][i_occ[k],:,:])
...
# Right Sweep ----------------------------
    ...
    for i in range(N-1):
        ...
        E,vl,vr = lib.eig(opt_fun,init_guess,precond,pick=pick_eigs,left=True)
