...
for i in range(len(s)):#-1,0,-1):
    try:
        ...
        EEorth [i,:,:] = calc_entanglement_all(mps_fname ,mpo=mpo,orth=True)
        print('EEorth',EEorth[i,0,int(N/2)])
        if left:
            EElorth[i,:,:] = calc_entanglement_all(lmps_fname,mpo=mpo,orth=True)
        ...
        print('Something didnt work with {}/{}'.format(i+1,len(s)))
# Save Results
