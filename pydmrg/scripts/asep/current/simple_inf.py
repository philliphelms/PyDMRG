...
mbd = 4#np.array([30,40,50,60,70,80,90,100,120,140,160,180,200,250,300,350,400,500])
...
if leftState:
    def callFunc(output,outputl):
        # Extract outputs
        (N ,E ,gap ,Eloc ,EE ,EEs ,vecs ,mps ,mpo ,env ,S ,cont ,conv ,iterCnt ,fid ) = output
        (Nl,El,gapl,Elocl,EEl,EEsl,vecsl,mpsl,mpol,envl,Sl,contl,convl,iterCntl,fidl) = outputl
        ...
        return '\tEloc2={:.15f}\trho=({:.15f},{:.15f})\tcurr={:.15f}'.format(Eloc,rhoL,rhoR,curr)
else:
    def callFunc(output,outputl):
        return ''
