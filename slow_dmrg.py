...
class simpleHeisDMRG:
    ...
    def __init__(self,L):
        ...
        for i in range(L):
            ...
            if going_up:
                newMat = arrays = np.zeros([self.d,max_ind_curr,max_ind_prev])
    ...
    def h_optimization(self,site,dir):
        h = np.einsum('ijk,jlmn,olp->mionkp',self.F[site-1],self.W(site),self.F[site]) # THIS IS THE SAME THING, SO IT TOO
        si,aim,ai,sip,aimp,aip = h.shape
        h = np.reshape(h,(si*aim*ai,sip*aimp*aip)) # TRY DIFFERENT EIGENVALUE METHODS AND STARTING GUESSES
        w,v = eig(h) # TRY DIFFERENT METHODS & STARTING GUESSES
        w = w[(w).argsort()] # TRY USING DIFFERENT EIGENVALUES?
        v = v[:,(w).argsort()]
        self.M[site] = np.reshape(v[:,0],(si,aim,ai)) # TRY ALTERING THE RESHAPING HERE???
        return w[0]
    ...
    def run_optimization(self):
        ...
        while not converged:
            ...
            for site in range(1,self.L)[::-1]:
                ...
                print('\t\tOptimized site {}: {}'.format(site,energy_vec_all[-1]))
            energy_vec.insert(len(energy_vec),np.min(energy_vec_all[-1-self.L:-1]))
