...
#-----------------------------------------------------------------------------
# For the TASEP model, this script calculates the current and cumulant 
...
for j in range(len(N_vec)):
    ...
    Evec = np.zeros(s_vec.shape,dtype=np.complex128)
    Evec_adj = np.zeros(s_vec.shape,dtype=np.complex128)
    EE = np.zeros(s_vec.shape,dtype=np.complex128)
