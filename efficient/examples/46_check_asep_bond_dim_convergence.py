...
CGF_ed = np.zeros((len(p),len(s),len(bd)),dtype=np.complex128)    
nPart_ed = np.zeros((len(p),len(s),len(bd)),dtype=np.complex128)
density_ed = np.zeros((len(p),len(s),len(bd),N),dtype=np.complex128)
for k in range(len(bd)):
    for i in range(len(p)):
        for j in range(len(s)):
            ...
            x.kernel()
            #CGF_ed[i,j,k] = x.exact_diag()
            #nPart_ed[i,j,k] = np.sum(x.ed.nv)
            ...
            #density_ed[i,j,k,:] = x.ed.nv
        np.savez('asep_bond_dim_check_N'+str(N)+'_data_p'+str(len(p))+'s'+str(len(s)),s=s,p=p,CGF=CGF,EE=EE,nPart=nPart,density=density,current=current)#CGF_ed=CGF_ed,nPart_ed=nPart_ed,density=density,density_ed=density_ed)
