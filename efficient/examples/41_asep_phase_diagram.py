...
for i in range(len(p)):
    for j in range(len(s)):
        ...
        np.savez('asep_original_'+'N'+str(N)+'_data_p'+str(len(p))+'s'+str(len(s)),s=s,p=p,CGF=CGF,EE=EE,nPart=nPart,current=current,CGF_ed=CGF_ed,nPart_ed=nPart_ed,density=density,density_ed=density_ed)
